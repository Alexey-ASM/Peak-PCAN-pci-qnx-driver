...
//    LOG(debug) ;
	...
	{
//		LOG(error) << "Could not attach PCI device";
		throw std::runtime_error("<E> Could not attach PCI device");
	}

	if (PCIGetPCANPorts() == false)
	{
//		LOG(error) << "Incorrect PCI PCAN device ports config";
		throw std::runtime_error("<E> Incorrect PCI PCAN device ports config");
	}
...
//    LOG(info) << "Bus: " << sBusType.c_str()
//              << " Base address: " << std:: hex << chipAddr_
//              << " Irq: " << irq_;
    ...
    switch(bitRate)
    {
        ...
        default:
//            LOG(error) << "Incompatible bit rate: " << bitRate;
            throw std::runtime_error("<E> Incompatible bit rate");
    }
...
void
ControllerFactory::DeleteController()
{
//    LOG(debug) ;
...
bool
ControllerFactory::PCIAttachDevice(const pci_vid_t nVendorID, const pci_did_t nDeviceID)
{
...
//            LOG(error) << "Can't attach CAN controller err code: " << pci_strerror(err);
        ...
        {
//            int_t pcieCapId = pci_device_find_capid(bdf_, 0x10);
//
//            if(pcieCapId != -1)
//            {
//            }
...
void
ControllerFactory::PCIDetachDevice()
{
//    LOG(debug) ;
...
bool 
ControllerFactory::PCIGetPCANPorts()
{
//    LOG(debug) ;
...
void
ControllerFactory::FinializeInterrupt()
{
    if (configAddr_ != 0)
    {
        *(std::uint16_t*)(configAddr_ + PCAN_ICR) = PCAN_ICR_MASK;

...
CanManager::CanManager(std::shared_ptr<CanController> canController, uint32_t nQueueSize)
 ...
 , filling_(true)
{
//    LOG(info) << "nQueueSize: " << nQueueSize;
    ...
    if(nQueueSize > 24) 
    {
        throw std::runtime_error("<E> Error buffer size");
    }
...
//        LOG(error) << "Initialization Error";
        throw std::runtime_error("<E> Controller initialization Error");
    }
...
CanManager::~CanManager()
{
    ...
    //LOG(info) << "Data receive thread done";
...
//  iofunc_notify(ctp, msg, notify_, trig, 0, NULL);
...
{
    //send empty message to unblock read functions before close connection

//  errno = EBADF;

    //MsgReply(ctp->rcvid, EOK , 0, 0);
...
{
//  iofunc_notify_remove(ctp, notify_);

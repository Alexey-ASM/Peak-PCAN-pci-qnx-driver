...
//    LOG(info) << "Finalizing...";
    ...
    if (0 != canManager)
    {
        ...
        std::cout << "Can manager object was deleted..." << std::endl;
    }
}
...
{
    std::cout << "Signal '" << strsignal(nSigno) << "' was caught" << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    if(nUid != 0)
    {
        if( seteuid( 0 ) == -1 )
        {
            std::cout << "Can't set eUID: " <<  geteuid() << " old: " << nUid << std::endl;
            exit(EXIT_FAILURE);
        }
    }

    std::string drvRegPrefix("/dev/");
    std::string drvRegName("can");
    ...
    while((option = getopt(argc, argv, "abr:B:d:htVs:")) != -1)
    {
        ...
        case 'r':
            ...
            if (chdir(optarg))
            {
                std::cout << "chroot can't change path!" << std::endl;
                exit(EXIT_FAILURE);
            }
            else
            {
...
//        if (logLevel != 0)
//        {
//            ENABLE_SHMEM_LOG(LogMessageLevel(logLevel));
//            std::this_thread::sleep_for(std::chrono::milliseconds(100));
//            LOG(info) << __progname << " " << AppVersion() << " " << DescribeModule();
//        }

//        const EiraTech::robotools::IniFile IniFile(optionsFileName);
        ...
        if ((dpp = dispatch_create()) == 0)
        {
            ...
            std::cout << "Unable to allocate dispatch handle: " << argv[0] << std::endl;
        ...
        else
        {
            ...
            {
                std::cout << "Unable to attach name: " << argv[0] << std::endl;
                exitStatus = EXIT_FAILURE;
            }
            else
            {
    ...
    {
        std::cout << "unexpected can manager error!" << std::endl;
        exitStatus = EXIT_FAILURE;
    }
